...
SELECT nombre, precio AS €, round(precio/0.98,2) AS'$' FROM producto;
...
-- 19. Llista el nom i el preu del producto més car. (Utilitzi fabricantessolament les clàusules ORDER BY i LIMIT). NOTA: Aquí no podria usar MAX(preu), necessitaria GROUP BY.
...
ON f.codigo = p.codigo_fabricante
WHERE f.nombre = 'Lenovo'; 
-- x 37. Retorna totes les dades dels productes que tenen el mateix preu que el producte més car del fabricador Lenovo. (Sense utilitzar INNER JOIN).
SELECT * 
FROM fabricante AS f
LEFT JOIN producto AS p
ON f.codigo = p.codigo_fabricante
